...
- [ ] **Step 4.1:** Add Employee login tests proving `/login` authenticates Employee credentials.
- [ ] **Step 4.2:** Add JWT response tests proving Employee login returns `ROLE_EMPLOYEE`.
- [ ] **Step 4.3:** Add JWT-protected request tests proving Employee tokens do not grant admin-only Employee CRUD access.
- [ ] **Step 4.4:** Extend `AuthUserUtil` and/or test authentication helpers only where the tests or near-term code require it.
...
- [ ] **Step 5.1:** Add supplemental Employee repository tests for edge cases (constraint violations, cascade behavior, JOINED inheritance boundary conditions).
- [ ] **Step 5.2:** Add mapper contract-completeness tests: safe DTO output across all DTO variants, `ROLE_EMPLOYEE` serialization, absence of password/API-key fields, and null-safety on optional fields.
- [ ] **Step 5.3:** Add cross-cutting service tests: update-with-unchanged-email skips uniqueness check, blank password preserved on update, null roles ignored.
- [ ] **Step 5.4:** Add list/query edge-case tests: maximum page size rejection, invalid filter field rejection (`password`, `roles`, `apikey`), unsupported operators on boolean fields, and type-mismatch error responses.
- [ ] **Step 5.5:** Add controller validation tests: malformed request bodies, size-limit violations, and invalid query/sort field rejection at the HTTP layer.
- [ ] **Step 5.6:** Run the full `./mvnw test` cycle and confirm all existing Admin and Client tests still pass under the hardened security configuration.
...
- **Task Document Link:** [[Employee-User-Entity-and-Role-step-1-security-hardening 1]]
...
- **Task Document Link:** [[Employee-User-Entity-and-Role-step-2-domain-contract 1]]
...
- **Task Document Link:** [[Employee-User-Entity-and-Role-step-3-crud-query-business-rules 1]]
